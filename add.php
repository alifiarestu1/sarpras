...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO barang_sarpras 
        (nama_barang, jenis, didapatkan, lokasi, warna, tanggal_terima, kondisi, jumlah, nomor_urut, satuan, ukuran, merk, keterangan, gambar) VALUES
        ('$nama_barang', '$jenis', '$didapatkan', '$lokasi', '$warna', '$tanggal_terima', '$kondisi', '$jumlah', '$nomor_urut', '$satuan', '$ukuran', '$merk', '$keterangan', '$gambar')";
    if ($koneksi->query($sql)) {
        $koneksi->query("INSERT INTO log_aktivitas (aksi, keterangan) VALUES ('Tambah','Menambah barang $nama_barang')");
        echo "<script>window.onload=function(){showToast('Data berhasil disimpan!'); setTimeout(function(){window.location='index.php';},1200);};</script>";
    } else {
        echo "<script>window.onload=function(){showToast('Gagal simpan data!',true);};</script>";
    }
}

// HybridField function dengan id unik
function hybridField($name, $label, $data) {
    static $uniq = 1;
    $selectId = $name . "_sel_" . $uniq;
    $inputId = $name . "_baru_" . $uniq;
    $uniq++;
    ?>
    <div class="form-group">
        <select name="<?= $name ?>" id="<?= $selectId ?>" onchange="showInput(this, '<?= $inputId ?>')" required>
...
<body>
    ...
    <h2>Tambah Data Sarpras</h2>
    <div class="form-container">
    <form method="post" enctype="multipart/form-data" class="form-main" autocomplete="off">
        <?php
        hybridField('nama_barang', 'Nama Barang', $namaList);
        hybridField('jenis', 'Jenis Barang', $jenisList);
        hybridField('didapatkan', 'Didapatkan', $didapatkanList);
        hybridField('lokasi', 'Lokasi', $lokasiList);
        hybridField('warna', 'Warna', $warnaList);
        ?>
        <div class="form-group">
            <input type="text" name="tanggal_terima" id="tanggal_terima" placeholder=" " required maxlength="7" pattern="\d{2}/\d{4}">
            <label for="tanggal_terima">Bulan/Tahun Diterima (MM/YYYY)</label>
        </div>
        <?php hybridField('kondisi', 'Kondisi', $kondisiList); ?>
        <div class="form-row">
            <div class="form-group" style="width:48%;display:inline-block;">
                <input type="number" name="jumlah" id="jumlah" placeholder=" " required>
                <label for="jumlah">Jumlah</label>
            </div>
            <?php hybridField('satuan', 'Satuan', $satuanList); ?>
        </div>
        <div class="form-row">
            <div class="form-group" style="width:48%;display:inline-block;">
                <input type="number" name="nomor_urut" id="nomor_urut" placeholder=" " required>
                <label for="nomor_urut">Nomor Urut</label>
            </div>
            <div class="form-group" style="width:48%;display:inline-block;">
                <input type="text" name="ukuran" id="ukuran" placeholder=" ">
                <label for="ukuran">Ukuran</label>
            </div>
        </div>
        <div class="form-group">
            <input type="text" name="merk" id="merk" placeholder=" ">
            <label for="merk">Merk</label>
        </div>
        <div class="form-group">
            <textarea name="keterangan" id="keterangan" rows="2" placeholder=" "></textarea>
    ...
    <script>
        function showInput(sel, inputId) {
            document.querySelectorAll('input[name="'+sel.name+'_baru"]').forEach(function(inp){
                inp.style.display="none"; inp.required=false;
            });
            var inputBaru = document.getElementById(inputId);
        ...
        document.querySelector("form").addEventListener("submit", function(e){
            ...
            if(!valid) {
                showToast("Isi semua field yang wajib!", true);
                e.preventDefault();
            }
        });
